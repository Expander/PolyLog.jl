...
function li4(x::Float64)::Float64
    ...
    elseif y < 0.9
        cp = (
            3.1862424089342765646e-9, 9.9999994512820440989e-1,
           -3.7747775757313358087e+0, 5.6768601579103074590e+0,
           -4.2980010983961213575e+0, 1.6926918997624899056e+0,
           -3.1696542409628207960e-1, 2.0368233561031260407e-2
        )
        cq = (
            1.0000000000000000000e+0, -3.8372780020702014975e+0,
            5.9043463224802909805e+0, -4.6235611716344668553e+0,
            1.9221731606149699277e+0, -3.9773415425963524321e-1,
            3.2672161963826069032e-2, -4.5557334754268696125e-4
        )
        p = cp[1] + y*cp[2] + y^2*(cp[3] + y*cp[4]) + y^4*(cp[5] + y*cp[6] + y^2*(cp[7] + y*cp[8]))
        q = cq[1] + y*cq[2] + y^2*(cq[3] + y*cq[4]) + y^4*(cq[5] + y*cq[6] + y^2*(cq[7] + y*cq[8]))
        r + s*p/q
    else # y <= 1.0
        l = log(y)
